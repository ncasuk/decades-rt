...
{    
   ...
   boolean movewhichone=false;
    ...
    xyplot1(float xmx,float xmn,float ymx,float ymn){
    ...
    g.setFont(new Font("SansSerif",Font.BOLD,fontSize));
       int i1=1;
        ...
        if(independentx){
            i1=0;
            i2=1;
            if(whichline==0){i2=1;}else{i2=whichline;}
...
//    setDatamaxmin(q.maxs[i1],q.mins[i1],q.maxs[i2],q.mins[i2]);
    ...
    if(whichline!=0)g.setColor(colors[colours[whichline-1]]);
    StringBuffer sb;
    ...
    if(whichline==0){
        ...
        v=getTickmarks((ymin-yc[whichline-1])/ym[whichline-1],
                       (ymax-yc[whichline-1])/ym[whichline-1]);
    }
    int iy=v.size()/2;
    ...
    if(whichline==0){
        ...
        d=convert(0,t.value*ym[whichline-1]+yc[whichline-1]);
     }
       g.drawString(t.name,5,d.height);
       System.out.println(i+" I  IY "+iy);
        if((i==iy)&&(whichline!=0)){g.drawString(getunits(i2),2,d.height+15);}
    }
   ...
   public void drawPoints(Graphics g,float[]xx,float[] yy,int i){
    ...
    if(independentx){
        ...
        System.out.println(ym[i-1]+"  "+yc[i-1]);
	...
	public void changezooms(){
	    ...
	    if(whichline!=0){
            if(historypos>mhistpos){
              ...
              System.out.println("Ymin0= "+mhist[3]+"  Ymax0= "+mhist[2]);
              float dy=(ymax-ymin);
